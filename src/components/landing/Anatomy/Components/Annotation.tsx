...
export default function Annotation({ step, progress }: AnnotationProps) {
  ...
  return (
    <>
      {/* ========================================= */}
      {/* 1. Desktop View Only (lg and up): Annotations with Lines */}
      {/* ========================================= */}
      <motion.div
      ...
      >
        <svg
          className={`absolute top-1/2 -translate-y-1/2 w-8 sm:w-16 md:w-12 lg:w-20 xl:w-32 h-2 overflow-visible ${
            isRight
        ...
        >
          ...
          <p className="text-[8px] sm:text-[10px] md:text-[11px] lg:text-xs xl:text-sm uppercase opacity-80 leading-tight font-medium drop-shadow-sm mt-1">
            {step.description}
          </p>
        </motion.div>
      </motion.div>
      ...
      >
        <div className="relative w-4 h-4 bg-[#FF5A00] rounded-full flex items-center justify-center shadow-lg">
          <div className="absolute inset-0 bg-[#FF5A00] rounded-full animate-ping opacity-75"></div>
